...
接下来编写一个通用的函数柯里化。`function currying(){}`接受一个参数，即将被 currying 的函数，在这个例子中这个函数的作用遍历本月每天的开销并求出他们的总和。
...
那么有没有办把泛化 this 的过程提取出来，`uncurrting`就是用来解决这个问题，一下代码是`uncurrting`的实现方法之一
